...
class SubNotifyhost extends BaseModel
{
    public function updateOrderStatusForBank($orderid, $receipt_amount, $is_buylist = 1)
    {
        ...
        if ($receipt_amount < ($orderlist_row['ordermoney'] + $orderlist_row['sub_money'])) {
            return false;
        }
        if ($receipt_amount >= $orderlist_row['ordermoney'] + $orderlist_row['sub_money']) {
        ...
        if ($cukun >= $orderlist_row['quantity']) {
            ...
            if ($res3) {
                ...
                if ($orderdefine_row['is_tel'] == '1' && !empty($orderdefine_row['tel'])) {
                    $this->send_sms($orderdefine_row['tel'], ['orderid' => $orderid], 'SMS_114380856');
                }
