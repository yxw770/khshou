...
class DlSubmit extends Controller
{
    private $debug = false;
    private $is_random = true;

    public function add($usergoodlist_sub_row)
    {
        if (request()->isPost()) {
            ...
            $usergoodlist_row = db('usergoodlist')->where(['goodlistid' => $usergoodlist_sub_row['parent_id']])->find();
            $is_wholesale = $usergoodlist_row['is_wholesale'];
            $price = $usergoodlist_sub_row['price'];
            ...
            $is_author = $usergoodlist_row['is_author'];
            $userid = $usergoodlist_row['userid'];
            $orderlist_load_arr = [];
            $orderdefine_load_arr = [];
            if ($is_author == '0') {
                return json_error('该商品已经关闭代理');
            }
            if ($is_trush == '1') {
            ...
            $coupon_amount = 0;
            if ($usergoodlist_sub_row['is_coupon'] == '1') {
                if (!empty($couponcode)) {
                    $usercoupon_row = db('usercoupon')->where(['userid' => $usergoodlist_sub_row['userid'], 'couponcode' => $couponcode,])->field(true)->find();
            ...
            if ($usergoodlist_sub_row['is_wholesale'] == '1') {
                $userwholesale_arr = db('userwholesale')->where(['userid' => $usergoodlist_sub_row['userid'], 'goodlistid' => $usergoodlist_sub_row['goodlistid'],])->field('define_quantity,define_price')->select();
            ...
            $ordermoney_const = $usergoodlist_row['price'] * $quantity;
            if ($is_email == '1') {
            ...
            if ($ordermoney < $ordermoney_const) {
                return json_error('商品价格过低，不能低于授权商品价格');
            }
            if ($this->is_random) {
                if (!$this->debug) {
            ...
            $orderlist_load_arr['goodlistid'] = $usergoodlist_row['goodlistid'];
            ...
            $orderlist_load_arr['ordermoney'] = $ordermoney_const;
            ...
            $orderlist_load_arr['is_sub'] = 1;
            $orderlist_load_arr['sub_userid'] = $usergoodlist_sub_row['userid'];
            $orderlist_load_arr['sub_money'] = $ordermoney - $ordermoney_const;
            Db::startTrans();
