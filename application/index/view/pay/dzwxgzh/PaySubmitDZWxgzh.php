...
//        echo $string;
     ...
     * 网页获取code
     */
    public function getCode_dianzhui($appid,$redirect_uri)
    {
        //1.获取到code
        if(!isset($_GET['code'])){
            $redirect_uri = urlencode($redirect_uri);//注意“网页帐号→网页授权获取用户基本信息→修改”修改www.laoo.club//?为什么我的网站用https就无法跳转过去呢
            $url = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=" . $appid . "&redirect_uri=" . $redirect_uri . '&response_type=code&scope=snsapi_base&state=STATE#wechat_redirect';
            header("Refresh: 1;url=" . $url);
            return false;
//            header("location:" . $url);//返回code的值，或者调用页面使用$code = $_GET['code'];获取code的值；
            exit();//必须停止掉程序，否则会继续执行并且请求，导致订单号重复，那会导致拿不到正确的值。
            //使用curl拿不到code的值
//            $res = $this->http_curl_dianzhui($url);
//            return $res;
        } else {
            return $_GET['code'];
        }
     ...
     * 获取openid
     */
    public function getOpenid_dianzhui($appid,$appsecret,$redirect_uri)
    {
        //1.获取到code
        $code=$this->getCode_dianzhui($appid, $redirect_uri);
    ...
    public function http_curl_dianzhui($url)
    {
        $ch = curl_init();
