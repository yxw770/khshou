...
class Settlement extends AdminBase
{
    public function index()
    {
        ...
        if (request()->isGet()) {
            ...
            if (isset($get['do']) && $get['do'] == 'search') {
                ...
                $userlist_arr = $userp_model->get_all_settlement_by_condition_on_admin($condition);
                $list = $userlist_arr[0];
                $page = $userlist_arr[1];
            } else {
                $userlist_arr = $userp_model->get_all_settlement_by_condition_on_admin($condition);
        ...
        $assign_arr = array('title' => "商户结算", 'list' => $list, 'page' => $page, 'takecashlowest' => $takecashlowest,);
    ...
    public function add()
    {
        ...
        $userpay_row = $userp_model->get_one_settlement_by_condition_on_admin($condition);
    ...
    public function addsave()
    {
        if (request()->isPost()) {
            ...
            $userpay_row = $userpay_model->get_add_payment_by_condition_on_admin($condition);
            ...
            $condition3 = ['userid' => $post['userid'], 'addtime' => ['<', $zero_time], 'is_selllist' => 1, 'is_frozen' => 0,];
